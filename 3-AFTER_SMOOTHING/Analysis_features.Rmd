...
end=which(rownames(TR_profiles) == "09.10.2023.23.45") ## test §§
...
# Genotype  3.2229e+14   313  4.0021 < 2.2e-16 ***
# Residuals 3.1905e+15 12401                    
```
...
To test the hypothesis, we take the OP8 dataset, which is smooth transpiration before LA. Do we have correct profiles?
How can we correct? Take planimeter measurements in the middle and at the end of the trial! 
...
```{r load data }
TR_smooth<-read.csv("C:/Users/2021lg003/Documents/LEASYSCAN_ANALYSIS/2-DATA_SMOOTHING/results/OP-8_wthr.ETref.ETobs.csv", sep = ";", header = TRUE, dec = ".")

colnames(TR_smooth)[9:ncol(TR_smooth)] <- sub("X","",colnames(TR_smooth)[9:ncol(TR_smooth)])
TR_smooth=TR_smooth[-c(1:8)]
timeline=colnames(TR_smooth[9:ncol(TR_smooth)])
timeline=dmy_hm(timeline)
...
for ( i in 1:ncol(TR_trial_period)) { 
  ...
  for ( j in 1:nrow(subset_geno_trt)) { 
    ...
    if(  (isTRUE(subset_geno_trt$TR[j]<0)) |  (isTRUE(subset_geno_trt$TR[j]> 500) ) == TRUE) {
