generateETrRatio <- function(x) {
  ...
  wthr.ETref.ETobs.Ratio <- as.data.frame(t(x[8:nrow(x), 7:ncol(x)]))
  ...
  for(j in 2:ncol(wthr.ETref.ETobs.Ratio))
  {
    ...
    wthr.ETref.ETobs.Ratio[, j] <- na.spline(wthr.ETref.ETobs.Ratio[, j])
  }
  
  ET_ratio_mat <- x
  
  ET_ratio_mat[9:nrow(ET_ratio_mat), 7:ncol(ET_ratio_mat) ] <- round(t(wthr.ETref.ETobs.Ratio[ ,-1]), 3)
