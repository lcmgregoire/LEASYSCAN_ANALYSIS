dataPART <- function(x) {
  ...
  by_unit <- ETr_smth_IP[-c(1:8) ,c(1, 7:ncol(ETr_smth_IP))]
  ...
  rownames(ETR_baseDF) <- colnames(ETr_smth_IP)[7:ncol(ETr_smth_IP)]
  ...
  ETR_baseDF$solarRAD <- as.numeric(as.character(ETr_smth_IP[4, 
                                                             7:ncol(ETr_smth_IP)])) ## Need to convert row to vector using c()
