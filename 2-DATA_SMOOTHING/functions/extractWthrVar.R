extractWthrVar <- function(x, y) {
  ...
  if (length(sen.cnt) > 2)
  {
    ...
    wthr.d <- wthr.tmp[,-1]
    wthr.tmp.med <- wthr.d %>% group_by(ts, timestamp, variable) %>% summarise_all(na.omit(median))
    ...
    wthr.var.DF <- wthr.tmp.med
    wthr.var.DF$sensor <- rep(sen.cnt[1], nrow(wthr.tmp.med))
    wthr.var.DF <- wthr.var.DF[,c(5,3,2,4,1)]
