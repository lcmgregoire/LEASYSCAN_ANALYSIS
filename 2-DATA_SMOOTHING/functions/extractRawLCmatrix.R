extractRawLCmatrix <- function(x, y, s, z, inter) {
  ...
  head(m.lc.df$timestamp)
  
  m.lc.df$ts <- m.lc.df$timestamp
  m.lc.df$ts <- ymd_hms(m.lc.df$ts) #Format here
  
  m.lc.df$date <- date(m.lc.df$ts)

  m.lc.df$time <- strftime(m.lc.df$ts, format="%H:%M:%S", tz="UTC")
  
  m.lc.df <- m.lc.df[m.lc.df$date >= s & m.lc.df$date <= z,]
  ...
  # select from Metadata: "unit", "old_unit","Treatment","Genotype, "G..Alias", "Replicates"
  sec.dtLenMETA <- y[y$unit %in%  sec.dtLen$sec, c(1,2,4,6,7,8)]
  ...
  m.lc.df <- m.lc.df[!is.na(m.lc.df$timestamp), ]
  ...
  # Create a sequence of time-stamps with user-defined interval for 1 day
  ...
  TS_base<-as.data.frame(as.character(seq(lubridate::ymd_hm(paste0(sec.dtLen$F.dt[1]," ",'00:00')),
                                          lubridate::ymd_hm(paste0(sec.dtLen$L.dt[1]," ",end.seq_hhmm)), 
                                          by = paste0(inter, " ", "mins"))))
  ...
  for(i in 1:nrow(m.lc.df))
  {
    if(! m.lc.df$time[i] %in% hms.ts.base)
    {
      ...
      m.lc.df$time[i] <- TS_base$time[j]} else {
        
        m.lc.df$time[i] <- m.lc.df$time[i]
      }
  ...
  # Set Last date as per experiment details: For Exp-41, 
  # last date = Plant Eye 3DLA data's last date

  L.dt <- z

  TS_ALL<-as.data.frame(as.character(seq(ymd_hm(paste0(F.dt," ",'00:00')),
                                         ymd_hm(paste0(L.dt," ",end.seq_hhmm)), 
                                         by = paste0(inter, " mins"))))
  ...
  hms.inter <- paste0("00:",inter,":00")
  
  for (n in 1:(nrow(sec.dtLen)))
  {
    tmp.lc <- m.lc.df[(m.lc.df$unit == sec.dtLen$sec[n] &

              (m.lc.df$date < L.dt && m.lc.df$TS.n > m.lc.df$TS.n[1] - lubridate::hms(hms.inter))),
  ...
  print(paste0("Loadcell matrix created in: ", round((b-a), 2), " sec"))
