getFeatureHe <- function(x, y, d, p) {
  ...
  for (i in 1:length(dates)){
    ...
    features.DF <- as.data.frame(cbind(raw.trans[9:nrow(raw.trans), 
                                                 1:6], tmp))
    ...
    for(f in 1:(ncol(features.DF)-6))
      
    {
      he.tmp <- features.DF[ , c(4, (f+6))] # 4 is Genotype-column and f+5 is the feature column
