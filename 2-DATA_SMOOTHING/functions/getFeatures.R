getFeatures <- function(x) {
  ...
  dd.ts <- spl.smth[9:nrow(spl.smth), 7:ncol(spl.smth)] 
  ...
  dd.meta <- spl.smth[1:8, 7:ncol(spl.smth)] 
  ...
  for(j in 1:ncol(spl.smth.df)){
    ...
    spl.smth.whtr.sec.df1 <- spl.smth.whtr.sec.df %>%
      arrange(date, time) %>%
      group_by(date) %>%
      mutate(Dmax = max(na.omit(Data)), Dmin = min(na.omit(Data))) 
    
    if(!(max(na.omit(spl.smth.whtr.sec.df1$Data))==0)) # condition to prevent error columns with all 0 values
    {
      for(i in 1:length(dates)){
        ...
        #3: slope.07-maxET
        et.07.loc <- which(lubridate::hms(data.tmp$time) == lubridate::hms("07:00:00"))
        ...
        #4: slope.00-07
        et.00.loc <- which(lubridate::hms(data.tmp$time) == lubridate::hms("00:00:00"))
        ...
        #5: slope.19-23:45
        et.19.loc <- which(lubridate::hms(data.tmp$time) == lubridate::hms("19:00:00"))
        et.23_45.loc <- which(lubridate::hms(data.tmp$time) == lubridate::hms("23:00:00"))
        d.tmp3<-data.frame(x3 = 1:(et.23_45.loc - (et.19.loc-1)), 
                           y3 = data.tmp$Data[et.19.loc:et.23_45.loc])
        ...
        #8: auc.10-15 
        y1.10 <- which(lubridate::hms(data.tmp$time) == lubridate::hms("10:00:00"))
        y1.15 <- which(lubridate::hms(data.tmp$time) == lubridate::hms("15:00:00"))
        ...
        #11: auc.07-19
        y1.07 <- which(lubridate::hms(data.tmp$time) == lubridate::hms("07:00:00"))
        y1.19 <- which(lubridate::hms(data.tmp$time) == lubridate::hms("19:00:00"))
        ...
        y1.w.loc <-  which(lubridate::hms(data.tmp$time) == lubridate::hms("10:00:00"))
        y2.w.loc <-  which(lubridate::hms(data.tmp$time) == lubridate::hms("15:00:00"))
        ...
        y3.w.loc <-  which(lubridate::hms(data.tmp$time) == lubridate::hms("00:00:00"))
        y4.w.loc <-  which(lubridate::hms(data.tmp$time) == lubridate::hms("06:00:00"))
        ...
        y5.w.loc <-  which(lubridate::hms(data.tmp$time) == lubridate::hms("20:00:00"))
        y6.w.loc <-  which(lubridate::hms(data.tmp$time) == lubridate::hms("23:00:00"))
